...
class Questions extends Admin_Controller
{
	...
	{  
		$search_name_keyword  =  isset($_POST['search_name'])?trim($_POST['search_name']):(isset($_SESSION['questions_search_name'])?$_SESSION['questions_search_name']:'');
		$this->session->set_userdata('questions_search_name', $search_name_keyword); 		
		$keyword_name_session = $this->session->userdata('questions_search_name');
		if($keyword_name_session != '')
		{
			$keyword_name = $this->session->userdata('questions_search_name');
		}
		else
		{
			isset($_SESSION['questions_search_name'])?$this->session->unset_userdata('questions_search_name'):'';
			$keyword_name = "";
		}
		...
		if($keyword_name)
		{
			$where[] = array( FALSE, "CONCAT(u.first_name,' ',u.last_name) LIKE '%".$keyword_name."%'" );
		...
		$join_tables[] = array('users u','u.id = e.user_id','LEFT');
 		$fields = 'e.id,e.queries,e.created,e.status,u.first_name,u.last_name';
		$data['total_rows'] = $config['total_rows'] = $this->base_model->get_advance_list('faq_queries e', $join_tables, $fields, $where, 'num_rows','','','e.id');
		$data['questions'] = $this->base_model->get_advance_list('faq_queries e', $join_tables, $fields, $where, '', 'e.id', 'desc', 'e.id', $limit_start, $limit_end);
	...
	{
		$this->session->unset_userdata('questions_search_name');
	...
	public function view($id = NULL){
		...
		$this->base_model->update_status($id, $status_data, 'faq_queries');
 		$join_tables[] = array('users u','u.id = e.user_id','LEFT');
 		$fields = 'e.id,e.queries,e.created,e.status,u.first_name,u.last_name';
		$where[] = array( TRUE, 'e.id', $id);
			$data['questions'] = $this->base_model->get_advance_list('faq_queries e', $join_tables, $fields, $where, '', 'e.id', 'desc', 'e.id', '','');

			$data['main_content'] = 'questions/view';
		  	$data['page_title']  = 'View Queries';  
			$this->load->view(ADMIN_LAYOUT_PATH, $data); 	
		}
	function update_status($id,$status,$pageredirect,$pageno)
	{
		$table_name = 'faq_queries';
	...
	function bulkactions($pageredirect=null,$pageno){
		...
		if($bulk_type == 1){
			foreach($bulk_ids as $id) {
				$data = array('status' => '1');
				$this->base_model->update_status($id, $data, 'faq_queries');
			}
		...
		else if($bulk_type == 3)
		{
			foreach($bulk_ids as $id) 
			{
				...
				$this->base_model->delete('faq_queries', array('id' => $id));
