...
class Course_video extends CI_Controller
{
	...
	public function add_view_count()
	{
		...
		{							
			...
			if (!$this->input->post()){
				$error = array(			
								"id" => "Please enter the course video id",
							);
				$MSG = implode("<br>",$error);
				$result = array( 'ST'=> 0 , 'MSG'=> $MSG);
				echo $response = json_encode($result);
			...
			$this->form_validation->set_rules('id', 'course video id','trim|required');
			
			if ($this->form_validation->run())
			{					
				$course_video_id = $this->input->post('id');
				...
				if($user_by_token)
				{
					...
					switch(trim($response))
					{
						case 'SUCCESS' :
							...
							{		
								$this->course_video_model->add_course_video_count($course_video_id);
